...
const SAINTH = () => {
  ...
  // const [audioLevel, setAudioLevel] = useState(0); // This state is declared but not updated or used. -> REMOVED

  const recognitionRef = useRef(null);
  const synthRef = useRef(null);
  // const animationRef = useRef(null); // This ref is declared but not used. -> REMOVED
  // const audioContextRef = useRef(null); // This ref is declared but not used. -> REMOVED
  // const analyserRef = useRef(null); // This ref is declared but not used. -> REMOVED
  ...
  const processCommand = async (input) => {
    ...
    if (lowerInput.includes('hello') || lowerInput.includes('привет') || lowerInput.includes('salam') || lowerInput.includes('ciao')) {
      ...
      response = currentLanguage === 'en' ? `All systems are operational. CPU is at ${systemStatus.cpu}%, network connection is excellent. ${devices.length} devices are connected and ready.` :
                currentLanguage === 'ru' ? `Все системы работают. ЦП на ${systemStatus.cpu}%, сетевое соединение отличное. ${devices.length} устройств подключено и готово.` :
                currentLanguage === 'az' ? `Bütün sistemlər işləyir. CPU ${systemStatus.cpu}%-də, şəbəkə əlaqəsi əla. ${devices.length} cihaz qoşulub və hazırdır.` :
                `Tutti i sistemi sono operativi. CPU al ${systemStatus.cpu}%, connessione di rete eccellente. ${devices.length} dispositivi connessi e pronti.`;
      setEmotionalState('neutral');
    } else {
